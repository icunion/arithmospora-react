 ...
 * @see connectSources
 */
export const useSources = (sources: string[], options = {}) => {
  useEffect(() => {
...
export const useStat = (
  ...
  group: string,
  stat: string,
  statSelector = (stateStat: Stat) => stateStat
) => {
...
export const useMilestone = (holdFor = 8000) => {
  ...
  useEffect(() => {
    if (lastMilestone.isNew) {
      milestoneCompleteTimeoutRef.current != null && window.clearTimeout(milestoneCompleteTimeoutRef.current)
      milestoneCompleteTimeoutRef.current = window.setTimeout(() => {
  ...
  useEffect(() => {
    return () => {
      if (milestoneCompleteTimeoutRef.current != null) {
        clearTimeout(milestoneCompleteTimeoutRef.current)
      }
    }
  }, [])
